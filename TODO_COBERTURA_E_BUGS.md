...
- [x] P1. Manter todo o catálogo consumido pelo site sincronizado no Supabase.
  ...
  - Evidência adicional (2026-09-07): `scripts/generate-catalog-seed.cjs` passou a preservar `item.mechanics` em `data` sem criar campos vazios; a migração completa atualizou 3.795/3.795 registros e o Supabase confirmou 415 magias, 93 com mecânicas estruturadas e 415 com estado de revisão explícito. `scripts/migrate-catalog-to-supabase.cjs` agora carrega `.env` silenciosamente quando necessário, evitando fallback acidental para a chave pública e bloqueio RLS.
- [ ] P1. Auditar e confirmar as 206 mecânicas pendentes usando os PDFs/TXT locais; manter `needs_review` quando a regra não puder ser confirmada.
