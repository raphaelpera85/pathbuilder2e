...
- [ ] Poderes e escolhas de classe por nível.
  - Progresso adicional: escolhas estruturais D&D 5e de Estilo de Luta (Guerreiro/Paladino/Patrulheiro), Metamagia e Dádiva do Pacto, além de escolhas T20 de escolas do Bardo, inimigo/terreno do Caçador, caminho do Cavaleiro e perícias de Especialista do Ladino, agora são catalogadas, exibidas no wizard/ficha, persistidas em `classChoices` e validadas por nível; outras escolhas situacionais de classe ainda precisam de grupos estruturados.
...
- [x] Completar seeds versionados para perícias, poderes/talentos, itens e magias.
  ...
  - Verificação remota atual: T20 (`padrao`) = 66 itens, 66 magias e 412 poderes; D&D 5e (`standard`) = 111 itens, 58 magias e 40 talentos; OSE (`advanced`) = 53 itens e 34 magias; PF2e permanece separado por ruleset.
...
- [x] Auditar contagens local × Supabase por tabela e por sistema.
  - Evidência: `npm run audit:core:supabase` confirmou por `system_id + ruleset` D&D 5e (9 raças, 12 classes, 13 antecedentes, 40 subclasses, 111 itens, 58 magias e 40 talentos), T20 (17 raças, 14 classes, 35 origens, 66 itens, 66 magias e 412 poderes) e OSE (10 ancestralidades, 16 classes, 53 itens e 34 magias); a auditoria também detectou e a migration `202609130002_reconcile_pf2e_subclass_orphans.sql` removeu 23 subclasses PF2e Remaster órfãs, deixando `orphanSubclasses = []`.
