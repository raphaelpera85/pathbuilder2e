...
- [ ] Importar a totalidade de armas, armaduras, escudos, itens gerais, kits e tesouro.
  - Progresso T20: tabelas 3-3, 3-4 e 3-5 do livro-base agora estão catalogadas em 112 itens com armas, armaduras, escudos, munições, itens gerais, alquimia, alimentação, animais, veículos e vestuário; preços, pesos, páginas e resumos estão estruturados em `t20Compendium.ts` e sincronizados pela migration `202609130028_expand_t20_equipment_catalog.sql`. Itens mágicos, serviços com cobrança por distância/noite e regras completas de modificações de itens superiores continuam pendentes.
...
- [ ] Completar preço, peso, dano, crítico, alcance e o catálogo integral.
  - Progresso T20: preços de armas, armaduras, escudos e equipamentos agora aparecem no construtor/inventário e são sincronizados no Supabase; os quatro pacotes de munição e o catálogo expandido de itens gerais do Livro Básico também foram adicionados; serviços variáveis e itens mágicos continuam pendentes.
...
- [ ] Importar magias completas com execução, alcance, alvo, duração, resistência, descrição e aprimoramentos.
  - Progresso T20: catálogo ampliado de 66 magias com círculo, tradição, escola, página e resumo de efeito conferido nas páginas 178–209; as 66 estão sincronizadas pela migration `202609130013_refresh_t20_spell_effects_complete.sql`. A disponibilidade segue as tabelas do Livro Básico: Arcanista/Clérigo nos níveis 1/5/9/13/17 e Bardo/Druida nos níveis 1/6/10/14. O Paladino não integra listas gerais: Orar (nível 2, p. 83) libera uma magia divina de 1º círculo por escolha e usa Sabedoria; a ficha registra escolhas repetidas, ajusta o limite de magias, mostra a quantidade no construtor/PDF e a valida. Limites de magias conhecidas também são aplicados, e o Arcanista inicial recebe três magias de 1º círculo. Texto integral e aprimoramentos ainda pendentes.
...
- [ ] Habilidades de classe, características por nível e efeitos completos das subclasses.
  ...
  - Progresso adicional: grupos de escolhas nucleares agora são estruturados, persistidos em `subclassChoices`, exibidos no wizard/ficha e validados (totens, terreno, manobras, disciplinas elementais, ancestralidade dracônica, surto selvagem e companheiro animal).
...
- [x] Seleção inicial de magias OSE limitada pelos espaços do 1º nível; Mago recebe Ler Magia automaticamente e demais conjuradores escolhem as magias preparadas.
...
- [x] Completar seeds versionados para perícias, poderes/talentos, itens e magias.
  ...
  - Verificação remota atual: T20 (`padrao`) = 112 itens, 66 magias e 412 poderes; D&D 5e (`standard`) = 206 itens, 301 magias e 40 talentos; OSE (`advanced`) = 53 itens e 34 magias; PF2e permanece separado por ruleset.
...
- [x] Auditar contagens local × Supabase por tabela e por sistema.
  - Evidência: `npm run audit:core:supabase` confirmou por `system_id + ruleset` D&D 5e (9 raças, 12 classes, 13 antecedentes, 40 subclasses, 206 itens, 301 magias e 40 talentos), T20 (17 raças, 14 classes, 35 origens, 112 itens, 66 magias e 412 poderes) e OSE (10 ancestralidades, 16 classes, 53 itens e 34 magias); a auditoria também detectou e a migration `202609130002_reconcile_pf2e_subclass_orphans.sql` removeu 23 subclasses PF2e Remaster órfãs, deixando `orphanSubclasses = []`.
