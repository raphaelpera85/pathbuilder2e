...
// 1. Replace the subclass initialization loop with the enriched subclasses catalog
const replacementGeneralCatalog = `// Subclasses e especializações de classes com metadados mecânicos ricos, perícias treinadas,
// talentos/ações concedidos, páginas de referência e descrições multilíngues precisas.
...
const ENRICHED_SUBCLASSES_CATALOG = {
  // === SWASHBUCKLER / ESPADACHIM (Player Core 2, pp. 100-101) ===
  ...
  // === ROGUE / LADINO (Player Core, pp. 168-169) ===
  "class.rogue.scoundrel": {
    ...
    summaries: {
      "pt-BR": "Especialista em lábia, distrações e fintas desconcertantes. Você é treinado em Enganação e Diplomacia e ganha o talento Finta Ágil. Ao Fintar com sucesso, o alvo fica desprevenido contra seus ataques corpo a corpo até o final do seu próximo turno.",
      en: "Master of silver-tongued bluffs and feints. Trained in Deception and Diplomacy and gains the Feint feat. When you successfully Feint, the target is off-guard against your melee attacks until the end of your next turn.",
  ...
  // === BARBARIAN / BÁRBARO (Player Core 2, pp. 48-51) ===
  ...
  "class.barbarian.instinto_draconico": {
    ...
    summaries: {
      ...
      en: "You channel the primordial might and fiery breath of dragons. Trained in Intimidação. Grants increased rage damage (+4 damage, +8 spec., +16 greater) matching your dragon type and resistance to that element during rage.",
  ...
  "class.barbarian.instinto_espiritual": {
    ...
    summaries: {
      "pt-BR": "Guiado por espíritos ancestrais que cercam sua arma de ectoplasma e energia mística. Treinado em Ocultismo ou Religião. Seus golpes durante a fúria causam dano espiritual e afetam criaturas incorpóreas normalmente (+3 dano, +7 esp., +13 maior).",
  ...
  "class.barbarian.instinto_da_furia_elemental": {
    ...
    rageDamage: { base: 2, spec: 5, greater: 12 },
    summaries: {
      "pt-BR": "Você canaliza o vórtice dos elementos primordiais. Treinado em Natureza. Infunde seus ataques com dano elemental e concede defesas e resistências adaptáveis durante a fúria (+2 dano, +5 esp., +12 maior).",
      en: "You channel the raw storm of primordial elemental planes. Trained in Nature. Infuses your strikes with elemental fury and grants defensive elemental resistances during rage (+2 damage, +5 spec., +12 greater).",
      es: "Canalizas el torbellino de los elementos primordiales. Entrenado en Naturaleza. Infunde tus ataques con daño elemental y otorga resistencias defensivas durante la furia (+2 daño, +5 esp., +12 mayor)."
    }
  },

  // === CHAMPION / CAMPEÃO (Player Core 2, pp. 58-62) ===
  "class.champion.retribuicao": {
    names: { "pt-BR": "Paladino (Retribuição)", en: "Paladin (Retributive Strike)", es: "Paladín (Golpe Retributivo)" },
    source: { book: "Livro do Jogador 2 (Player Core 2, Remaster)", page: 58 },
    trainedSkills: ["religion"],
    grants: ["action.retributive_strike"],
    summaries: {
      "pt-BR": "Campeão da justiça inflexível e virtude sacra. Concede a reação Golpe Retributivo (Retributive Strike) para proteger aliados e golpear imediatamente o agressor.",
      en: "Champion of righteous justice. Grants the Retributive Strike reaction to protect allies and punish aggressors immediately.",
      es: "Campeón de la justicia implacable. Concede la reacción Golpe Retributivo para proteger aliados y contraatacar agresores."
    }
  },
  "class.champion.misericordia": {
    names: { "pt-BR": "Redentor (Misericórdia)", en: "Redeemer (Glimpse of Redemption)", es: "Redentor (Atisbo de Redención)" },
    source: { book: "Livro do Jogador 2 (Player Core 2, Remaster)", page: 59 },
    trainedSkills: ["religion"],
    grants: ["action.glimpse_of_redemption"],
    summaries: {
      "pt-BR": "Campeão da compaixão e redenção. Concede a reação Vislumbre da Redenção para oferecer ao agressor a escolha de poupar o dano ou sofrer penalidades paralisantes.",
      en: "Champion of mercy and forgiveness. Grants Glimpse of Redemption reaction offering foes a choice between mercy or debilitating penalties.",
      es: "Campeón de la compasión y el perdón. Concede la reacción Atisbo de Redención ofreciendo piedad o penalizaciones graves."
    }
  },
  "class.champion.liberdade": {
    names: { "pt-BR": "Libertador (Liberdade)", en: "Liberator (Liberating Step)", es: "Libertador (Paso Liberador)" },
    source: { book: "Livro do Jogador 2 (Player Core 2, Remaster)", page: 59 },
    trainedSkills: ["religion"],
    grants: ["action.liberating_step"],
    summaries: {
      "pt-BR": "Campeão da autonomia e liberdade contra grilhões. Concede a reação Passo Libertador para reduzir dano a aliados e permitir fuga de agarramentos e efeitos restritivos.",
      en: "Champion of freedom and liberation. Grants Liberating Step reaction to mitigate ally damage and grant immediate escape from grabs and restraints.",
      es: "Campeón de la libertad y autonomía. Concede la reacción Paso Liberador para reducir daño a aliados y permitir escape inmediato de agarres."
    }
  },
  "class.champion.tiranos_desesperadores": {
    names: { "pt-BR": "Profano (Tiranos/Desesperadores)", en: "Unholy Champion (Tyrant / Desecrator)", es: "Profano (Tirano / Profanador)" },
    source: { book: "Livro do Jogador 2 (Player Core 2, Remaster)", page: 60 },
    trainedSkills: ["intimidation"],
    summaries: {
      "pt-BR": "Campeão profano que subjuga oponentes com opressão de ferro, dor implacável e destruição dos inocentes.",
      en: "Unholy champion subjugating foes through iron oppression, agonizing retribution, and destructive fury.",
      es: "Campeón profano que subyuga enemigos mediante opresión despiadada y dolor retributivo."
    }
  },

  // === BARD / BARDO (Player Core, pp. 96-98) ===
  "class.bard.enigma": {
  ...
  // === CLERIC / CLÉRIGO (Player Core, pp. 110-111) ===
  "class.cleric.cloistered": {
    ...
    summaries: {
      ...
      es: "Enfoque devoto en la liturgia sagrada y la magia divina. Entrenado en Religión y la habilidad de la deidad. Concede la dote Iniciado del Dominio, salvaciones de Voluntad Experto a nivel 1 y progresión acelerada de CD divina."
    }
  },
  ...
  // === DRUID / DRUIDA (Player Core, pp. 128-130) ===
  ...
  // === ALCHEMIST / ALQUIMISTA (Player Core 2, pp. 32-34) ===
  ...
  // === RANGER / PATRULHEIRO (Player Core, pp. 154-155) ===
  ...
  // === GUNSLINGER / PISTOLEIRO (Guns & Gears, pp. 108-112) ===
  ...
  // === INVESTIGATOR / INVESTIGADOR (Player Core 2, pp. 84-86) ===
  ...
  // === KINETICIST / CINETICISTA (Rage of Elements, pp. 12-20) ===
  ...
  // === THAUMATURGE / TAUMATURGO (Dark Archive, pp. 38-44) ===
  ...
  // === PSYCHIC / PSÍQUICO (Dark Archive, pp. 14-22) ===
  ...
  // === SORCERER / FEITICEIRO (Player Core 2, pp. 132-136) ===
  ...
  "class.sorcerer.linhagem_angelica": {
    ...
    tradition: "divine",
    summaries: {
      "pt-BR": "A graça celestial dos reinos sagrados abençoa suas palavras. Tradição Divina. Treinado em Religião. Concede magias de cura angelical e halo radiante protetor.",
  ...
  // === FIGHTER / GUERREIRO (Player Core, pp. 138-140) ===
  ...
  // === MONK / MONGE (Player Core 2, pp. 92-95) ===
  ...
  // === SUMMONER / CONVOCADOR (Secrets of Magic, pp. 54-59) ===
  ...
  "class.exemplar.mountain_s_scar": {
  ...
  "class.exemplar.icones_e_epitetos": {
    names: { "pt-BR": "Ícones e Epítetos Divinos", en: "Icons and Epithets", es: "Iconos y Epítetos" },
    source: { book: "Guerra dos Imortais (War of Immortals)", page: 16 },
    summaries: {
      "pt-BR": "Manifestações da centelha divina que personificam feitos lendários e poderes de divindades em ascensão.",
      en: "Manifestations of the divine spark embodying legendary deeds and rising deific powers.",
      es: "Manifestaciones de la chispa divina que personifican hazañas legendarias y poderes en ascenso."
    }
  },

  // === ANIMIST (War of Immortals, pp. 34-38) ===
  "class.animist.liturgista": {
    names: { "pt-BR": "Liturgista", en: "Liturgist", es: "Liturgista" },
    source: { book: "Guerra dos Imortais (War of Immortals)", page: 34 },
    trainedSkills: ["religion"],
    summaries: {
      "pt-BR": "Animista litúrgico que organiza rituais sagrados e canaliza espíritos divinos com orações estruturadas e bônus de sustentação.",
      en: "Liturgical animist structuring sacred rites and channeling divine apparitions through communal ceremonies.",
      es: "Animista litúrgico que organiza ritos sagrados y canaliza apariciones divinas a través de ceremonias."
    }
  },
  "class.animist.medium": {
    names: { "pt-BR": "Médium", en: "Medium", es: "Médium" },
    source: { book: "Guerra dos Imortais (War of Immortals)", page: 35 },
    trainedSkills: ["occultism"],
    summaries: {
      "pt-BR": "Canalizador psíquico que permite a espíritos inquietos habitarem temporariamente seu corpo para desferir ataques espirituais.",
      en: "Psychic conduit allowing restless spirits to temporarily possess their body to unleash spiritual attacks.",
      es: "Conducto psíquico que permite a espíritus inquietos habitar temporalmente su cuerpo para ataques espirituales."
    }
  },
  "class.animist.xama": {
    names: { "pt-BR": "Xamã", en: "Shaman", es: "Chamán" },
    source: { book: "Guerra dos Imortais (War of Immortals)", page: 36 },
    trainedSkills: ["nature"],
    summaries: {
      "pt-BR": "Guardião espiritual primal que se comunica com espíritos de rios, montanhas e florestas antigas.",
      en: "Primal spirit guardian communicating directly with ancient forest and mountain spirits.",
      es: "Guardián espiritual primigenio que se comunica con espíritus de la naturaleza y bosques antiguos."
    }
  }
};

// Aliases for legacy and alternative keys
ENRICHED_SUBCLASSES_CATALOG["class.exemplar.legacy_alias.exemplar_exemplar.gleaming_blade"] = ENRICHED_SUBCLASSES_CATALOG["class.exemplar.gleaming_blade"];
ENRICHED_SUBCLASSES_CATALOG["class.exemplar.legacy_alias.exemplar_exemplar.mountain_s_scar"] = ENRICHED_SUBCLASSES_CATALOG["class.exemplar.mountain_s_scar"];
ENRICHED_SUBCLASSES_CATALOG["class.exemplar.legacy_alias.exemplar_exemplar.radiant_halo"] = ENRICHED_SUBCLASSES_CATALOG["class.exemplar.radiant_halo"];
ENRICHED_SUBCLASSES_CATALOG["class.exemplar.legacy_alias.exemplar_exemplar.celestial_beast"] = ENRICHED_SUBCLASSES_CATALOG["class.exemplar.celestial_beast"];
...
for (const [classKey, classRecord] of Object.entries(PF2E_DATA.classes || {})) {
  for (const rawSubclass of (classRecord.subclasses || [])) {
    ...
    // Check enriched catalog by several slug variants
    const catalogKey1 = \`\${classId}.\${slug.split('.').slice(1).join('.')}\`;
    const catalogKey2 = \`\${classId}.\${en.normalize("NFD").replace(/[\\u0300-\\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "_")}\`;
    const catalogKey3 = \`\${classId}.\${pt.normalize("NFD").replace(/[\\u0300-\\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "_")}\`;
    const cleanClassId = classId.replace(/^class\\./, "").replace(/\\.legacy_alias.*$/, "");
    const catalogKey4 = \`class.\${cleanClassId}.\${en.normalize("NFD").replace(/[\\u0300-\\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "_")}\`;
    const catalogKey5 = \`class.\${cleanClassId}.\${pt.normalize("NFD").replace(/[\\u0300-\\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "_")}\`;

    const enriched = ENRICHED_SUBCLASSES_CATALOG[catalogKey1] || ENRICHED_SUBCLASSES_CATALOG[catalogKey2] || ENRICHED_SUBCLASSES_CATALOG[catalogKey3] || ENRICHED_SUBCLASSES_CATALOG[catalogKey4] || ENRICHED_SUBCLASSES_CATALOG[catalogKey5];
    ...
    const subclassItem = {
      ...
      name: enriched?.names?.["pt-BR"] ? \`\${enriched.names["pt-BR"]} (\${enriched.names.en})\` : \`\${pt} (\${en})\`,
      names: enriched?.names || { "pt-BR": pt, en, es: en },
      summaries: enriched?.summaries || {
      ...
      ruleset: enriched ? "remaster" : (classRecord.ruleset || "needs_review"),
      needs_review: enriched ? false : true,
      source: enriched?.source || (classRecord?.source?.book && Number.isInteger(classRecord.source.page) ? { book: classRecord.source.book, page: classRecord.source.page } : undefined),
      sourceApproximate: enriched ? false : true
    };

    if (enriched?.trainedSkills) subclassItem.trainedSkills = enriched.trainedSkills;
    if (enriched?.styleSkill) subclassItem.styleSkill = enriched.styleSkill;
    if (enriched?.grants) subclassItem.grants = enriched.grants;
    if (enriched?.armorProf) subclassItem.armorProf = enriched.armorProf;
    if (enriched?.rageDamage) subclassItem.rageDamage = enriched.rageDamage;
    if (enriched?.tradition) subclassItem.tradition = enriched.tradition;

    PF2E_DATA.subclasses.push(subclassItem);
  }
}
`;

const replacementChampionCauses = `// Player Core 2, pp. 90–93: causas remasterizadas do Campeão. Os quatro
...
const PLAYER_CORE_2_CHAMPION_CAUSES = [
  {
    slug: "esplendor",
    pt: "Causa do Esplendor",
    en: "Cause of Splendor",
    es: "Causa del Esplendor",
    page: 90,
    sanctification: "sagrado",
    trainedSkills: ["diplomacy"],
    summaries: {
      "pt-BR": "Você personifica a luz solar magnífica, a beleza inspiradora e a glória divina em batalha, cegando aqueles que ousam desafiar sua virtude. Treinado em Diplomacia.",
      en: "You embody magnificent solar light, inspiring beauty, and divine glory in battle, dazzling those who dare challenge your virtue. Trained in Diplomacy.",
      es: "Personificas la luz solar magnífica, la belleza inspiradora y la gloria divina en combate, cegando a quienes desafían tu virtud. Entrenado en Diplomacia."
    }
  },
  {
    slug: "iniquidade",
    pt: "Causa da Iniquidade",
    en: "Cause of Iniquity",
    es: "Causa de la Iniquidad",
    page: 90,
    sanctification: "profano",
    trainedSkills: ["deception"],
    summaries: {
      "pt-BR": "Você explora impiedosamente fraquezas morais, enganações e vícios mortais para quebrar a honra de seus adversários. Treinado em Enganação.",
      en: "You ruthlessly exploit moral vulnerabilities, deceit, and vices to erode your foes' integrity and resolve. Trained in Deception.",
      es: "Explotas despiadadamente debilidades morales, engaños y vicios mortales para quebrar la integridad enemiga. Entrenado en Engaño."
    }
  },
  {
    slug: "libertacao",
    pt: "Causa da Libertação",
    en: "Cause of Liberation",
    es: "Causa de la Liberación",
    page: 91,
    sanctification: "sagrado ou profano",
    trainedSkills: ["religion"],
    grants: ["action.liberating_step"],
    summaries: {
      "pt-BR": "Você quebra correntes de tirania e defende a autonomia absoluta. Concede a reação Passo Libertador (Liberating Step) para reduzir o dano sofrido por aliados e permitir fuga imediata de agarrões ou restrições.",
      en: "You shatter chains of tyranny and champion absolute freedom. Grants the Liberating Step reaction to reduce ally damage and allow immediate escape from grabs and restraints.",
      es: "Rompes cadenas de tiranía y defiendes la libertad absoluta. Concede la reacción Paso Liberador para reducir el daño a aliados y permitir escape inmediato de agarres."
    }
  },
  {
    slug: "justica",
    pt: "Causa da Justiça",
    en: "Cause of Justice",
    es: "Causa de la Justicia",
    page: 91,
    sanctification: "sagrado ou profano",
    trainedSkills: ["religion"],
    grants: ["action.retributive_strike"],
    summaries: {
      "pt-BR": "Você defende a lei suprema e pune aqueles que ferem os inocentes. Concede a reação Golpe Retributivo (Retributive Strike) para proteger um aliado próximo e desferir um contra-ataque imediato contra o agressor.",
      en: "You uphold righteous law and punish those who harm the innocent. Grants the Retributive Strike reaction to shield a nearby ally and immediately strike back at the attacker.",
      es: "Defiendes la ley sagrada y castigas a quienes dañan a los inocentes. Concede la reacción Golpe Retributivo para proteger a un aliado y contraatacar de inmediato."
    }
  },
  {
    slug: "obediencia",
    pt: "Causa da Obediência",
    en: "Cause of Obedience",
    es: "Causa de la Obediencia",
    page: 92,
    sanctification: "sagrado ou profano",
    trainedSkills: ["intimidation"],
    summaries: {
      "pt-BR": "Você impõe ordem rígida, hierarquia divina e disciplina militar implacável sobre seus inimigos. Treinado em Intimidação.",
      en: "You enforce rigid hierarchy, divine law, and relentless military discipline upon your foes. Trained in Intimidation.",
      es: "Impones jerarquía estricta, ley divina y disciplina implacable sobre tus enemigos. Entrenado en Intimidación."
    }
  },
  {
    slug: "profanacao",
    pt: "Causa da Profanação",
    en: "Cause of Desecration",
    es: "Causa de la Profanación",
    page: 92,
    sanctification: "profano",
    trainedSkills: ["religion"],
    summaries: {
      "pt-BR": "Você espalha blasfêmia, conspurcação e ruína abissal, infligindo dor atroz sobre quem ousa atacá-lo. Treinado em Religião.",
      en: "You spread defilement, blasphemy, and ruin, inflicting agonizing damage upon any foolish enough to strike you. Trained in Religion.",
      es: "Siembras blasfemia, profanación y ruina, infligiendo dolor atroz a cualquiera que ose atacarte. Entrenado en Religión."
    }
  },
  {
    slug: "redencao",
    pt: "Causa da Redenção",
    en: "Cause of Redemption",
    es: "Causa de la Redención",
    page: 93,
    sanctification: "sagrado",
    trainedSkills: ["religion"],
    grants: ["action.glimpse_of_redemption"],
    summaries: {
      "pt-BR": "Você oferece misericórdia e restauração pacificadora aos corações violentos. Concede a reação Vislumbre da Redenção (Glimpse of Redemption) para forçar agressores a escolher entre não causar dano ou sofrer penalidades paralisantes.",
      en: "You offer mercy, forgiveness, and pacifying restoration to violent souls. Grants the Glimpse of Redemption reaction forcing foes to choose between sparing their target or suffering debilitating penalties.",
      es: "Ofreces piedad y redención pacificadora a las almas violentas. Concede la reacción Atisbo de Redención para forzar a los agresores a no causar daño o sufrir penalizaciones."
    }
  }
];
for (const cause of PLAYER_CORE_2_CHAMPION_CAUSES) {
  const id = \`subclass.class.champion.cause_\${cause.slug}\`;
  const existing = (PF2E_DATA.subclasses || []).find((record) => record.id === id);
  if (existing) {
    existing.names = { "pt-BR": cause.pt, en: cause.en, es: cause.es };
    existing.summaries = cause.summaries;
    existing.trainedSkills = cause.trainedSkills;
    if (cause.grants) existing.grants = cause.grants;
    existing.source = { book: PLAYER_CORE_2_SOURCE, page: cause.page };
    existing.sourceApproximate = false;
    existing.ruleset = "remaster";
  ...
  PF2E_DATA.subclasses.push({
    ...
    className: "Campeão",
    name: \`\${cause.pt} (\${cause.en})\`,
    names: { "pt-BR": cause.pt, en: cause.en, es: cause.es },
    summaries: cause.summaries,
    causeId: \`champion.\${cause.slug}\`,
    sanctification: cause.sanctification,
    trainedSkills: cause.trainedSkills,
    grants: cause.grants,
    source: { book: PLAYER_CORE_2_SOURCE, page: cause.page },
...
// Apply replacement 1 (General subclasses)
const lines = code.split('\n');
const start1 = lines.findIndex((l) => l.includes('Subclasses eram mantidas apenas como strings') || l.includes('Subclasses e especializações de classes com metadados'));
const end1 = lines.findIndex((l) => l.includes('Livro do Jogador Remaster, pp. 114-115: o Patrono'));

if (start1 === -1 || end1 === -1) {
  console.error('Failed to locate block 1', { start1, end1 });
  process.exit(1);
}

const midLines = [
  ...lines.slice(0, start1),
  replacementGeneralCatalog,
  ...lines.slice(end1)
];

code = midLines.join('\n');

// Apply replacement 2 (Champion causes)
const lines2 = code.split('\n');
const start2 = lines2.findIndex((l) => l.includes('Player Core 2, pp. 90–93: causas remasterizadas do Campeão'));
const end2 = lines2.findIndex((l) => l.includes('Player Core 2, pp. 89 e 256-257: a progressão do Campeão é marcial'));

if (start2 === -1 || end2 === -1) {
  console.error('Failed to locate block 2', { start2, end2 });
  process.exit(1);
}

const finalLines = [
  ...lines2.slice(0, start2),
  replacementChampionCauses,
  ...lines2.slice(end2)
];

fs.writeFileSync(filePath, finalLines.join('\n'), 'utf8');
console.log('Successfully updated pf2e_data.js with complete enriched subclasses and Champion causes!');
