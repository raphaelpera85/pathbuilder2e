...
function ViewportSignals() {
  useEffect(() => {
    ...
    const updateSignals = () => {
      ...
      const portalNavEl = document.querySelector<HTMLElement>(".portal-nav");
      const portalNavHeight = (portalNavEl && portalNavEl.offsetParent !== null) ? portalNavEl.getBoundingClientRect().height : 0;
      const mobileNavEl = document.querySelector<HTMLElement>(".pb-mobile-view-nav");
      const mobileNavHeight = (mobileNavEl && mobileNavEl.offsetParent !== null) ? mobileNavEl.getBoundingClientRect().height : 0;
      document.documentElement.style.setProperty("--pb-mobile-chrome-height", `${topbarHeight + portalNavHeight + mobileNavHeight}px`);
    };
    updateSignals();
    const resizeObserver = typeof ResizeObserver !== "undefined" ? new ResizeObserver(updateSignals) : null;
    document.querySelectorAll<HTMLElement>(".pb-topbar, .portal-nav, .pb-mobile-view-nav").forEach((element) => resizeObserver?.observe(element));
