...
declare const PF2E_DATA: any;

export type PdfLocale = "pt-BR" | "en" | "es";
...
export const PDF_SIZES: Record<string, Record<PdfLocale, string>> = {
...
export const PDF_ALIGNMENTS: Record<string, Record<PdfLocale, string>> = {
...
export const PDF_TRAITS_DICT: Record<string, Record<PdfLocale, string>> = {
...
export const PDF_CATALOG_DICT: Record<string, Record<PdfLocale, string>> = {
  // Classes
...
export const PDF_DAMAGE_TYPES_DICT: Record<string, Record<PdfLocale, string>> = {
...
export function cleanWeaponName(name: string | undefined | null, locale: PdfLocale): string {
  if (!name) return "";
...
export function localizeCatalogItem(rawName: string | undefined | null, locale: PdfLocale): string {
  if (!rawName) return "";
  const trimmed = String(rawName).trim();
  if (PDF_CATALOG_DICT[trimmed]?.[locale]) {
  ...
  const data = typeof PF2E_DATA !== "undefined" ? PF2E_DATA : (globalThis as any).PF2E_DATA;
  if (data) {
    ...
    for (const col of collections) {
      if (!col) continue;
      const found = (Array.isArray(col) ? col : Object.values(col)).find((entry: any) => {
...
export function localizeTrait(trait: string | undefined | null, locale: PdfLocale): string {
  if (!trait) return "";
  const trimmed = String(trait).trim();
  const lower = trimmed.toLowerCase();
  if (PDF_TRAITS_DICT[lower]?.[locale]) return PDF_TRAITS_DICT[lower][locale];
...
export function localizeDamageType(dt: string | undefined | null, locale: PdfLocale): string {
  if (!dt) return "";
  const lower = String(dt).toLowerCase().trim();
  return PDF_DAMAGE_TYPES_DICT[lower]?.[locale] || dt;
}

export function localizeSense(sense: string | undefined | null, locale: PdfLocale): string {
  if (!sense) return "";
  return localizeCatalogItem(sense, locale);
}

export function getLocalizedFeatDetails(featNameOrId: string | undefined | null, locale: PdfLocale) {
  ...
  if (data && data.feats && Array.isArray(data.feats)) {
    ...
    const found = data.feats.find((f: any) => {
      if (!f) return false;
    ...
    if (found) {
      if (found.source?.book) {
        notes = `${found.source.book}${found.source.page ? ` p.${found.source.page}` : ""}`;
      }
      if (Array.isArray(found.traits)) {
        traits = found.traits.map((t: string) => localizeTrait(t, locale)).join(", ");
      }
      description = found.summaries?.[locale] || found.description || "";
    }
  }
...
export async function fillCharacterPdfForm(
  ...
  const locale: PdfLocale = (typeof options === "string" ? (options as PdfLocale) : options?.locale) || character.locale || (typeof window !== "undefined" && (window.localStorage?.getItem("pathbuilder.locale") as any)) || "pt-BR";
  ...
  const locSize = PDF_SIZES[rawSize]?.[locale] || rawSize;
  setTxt("Size", locSize);
  setTxt("Deity or Philosophy", character.deity || "");
  const rawAlignment = character.alignment || "Neutro";
  const locAlignment = PDF_ALIGNMENTS[rawAlignment]?.[locale] || rawAlignment;
  ...
  const acTotal = typeof calc.ac === "object" ? (calc.ac?.total ?? 10) : (calc.ac || 10);
  setTxt("AC", acTotal);
  setTxt("AC CALCULATION 1 DEXTERITY", formatMod(calc.ac?.dex !== undefined ? calc.ac.dex : mods.dex));
  const armorRank = character.armorProficiencies?.[equippedArmor.category] || "Treinado";
  const armorProfBonus = calc.ac?.prof !== undefined ? calc.ac.prof : getProfBonus(armorRank, level);
  setTxt("AC CALCULATION 2 PROFICIENCY", armorProfBonus);
  setTxt("AC CALCULATION 3 ITEM", calc.ac?.item !== undefined ? calc.ac.item : (equippedArmor.acBonus || 0));

  // Escudo
  const shieldBonus = calc.ac?.shield !== undefined ? calc.ac.shield : (character.shieldRaised ? (character.shieldBonus || 2) : 0);
  ...
  const formatPdfCondition = (c: any) => {
    if (!c) return "";
  ...
  setTxt("RESISTANCE AND IMMUNITIES", charResistances.map((r: any) => typeof r === "object" ? `${localizeCatalogItem(r.type || r.name, locale)} ${r.value || ""}`.trim() : localizeCatalogItem(String(r), locale)).join(", "));
  ...
  setTxt("CONSTITUTION", formatMod(mods.con));
  setTxt("PROFICIENCY", saves.fortitude.prof ?? getProfBonus(saves.fortitude.rank, level));
  ...
  setTxt("DEXTERITY", formatMod(mods.dex));
  setTxt("PROFICIENCY2", saves.reflex.prof ?? getProfBonus(saves.reflex.rank, level));
  ...
  setTxt("WISDOM", formatMod(mods.wis));
  setTxt("PROFICIENCY3", saves.will.prof ?? getProfBonus(saves.will.rank, level));
  ...
  // ----------------------------------------------------
  const percRank = character.perceptionRank || calc.perception?.rank || "Treinado";
  setTxt("PERCEPTION", formatMod(calc.perception?.total ?? (mods.wis + getProfBonus(percRank, level))));
  setTxt("PERCEPTION WISDOM", formatMod(mods.wis));
  const percProf = calc.perception?.prof ?? getProfBonus(percRank, level);
  setTxt("PERCEPTION PROFICIENCY", percProf);
  setTxt("PERCEPTION ITEM", calc.perception?.item || character.itemBonuses?.perception || 0);
  ...
  const sensesList = (Array.isArray(character.senses)
    ...
    .map((s: any) => localizeSense(String(s), locale))
    .join(", ");
  ...
  // ----------------------------------------------------
  const keyAttr = (calc.keyAttribute || (character as any).keyAbility || "dex").slice(0, 3).toLowerCase();
  const defaultKeyMod = mods[keyAttr as keyof typeof mods] !== undefined ? mods[keyAttr as keyof typeof mods] : mods.dex;
  ...
  const classDcTotal = classDcObj?.total || calc.classDc || (10 + defaultKeyMod + getProfBonus("Treinado", level));
  setTxt("CLASS DC", classDcTotal);
  setTxt("CLASS DC KEY", formatMod(classDcObj?.key !== undefined ? classDcObj.key : defaultKeyMod));
  ...
  for (const [skKey, meta] of Object.entries(skillsMap)) {
    const sk = calc.skills?.[skKey] || { total: 0, rank: "Destreinado", profBonus: 0, itemBonus: 0, penalty: 0 };
  ...
  meleeStrikes.slice(0, 3).forEach((st: any, idx: number) => {
    ...
    const locTraitsStr = (st.traits || []).map((t: string) => localizeTrait(t, locale)).join(", ");
    const isFinesse = (st.traits || []).some((t: string) => /finesse|acurada|acuidade/i.test(t));
  ...
  if (!backgroundSkillFeat && character.background) {
    ...
    const bgEntry = bgList.find((b: any) => b.name === character.background || b.id === character.background || (b.names && Object.values(b.names).includes(character.background)));
    if (bgEntry?.feat) backgroundSkillFeat = bgEntry.feat;
  }
  ...
  const classFeatures = Array.isArray(character.classFeatures) && character.classFeatures.length
    ? character.classFeatures.map((f: any) => typeof f === "string" ? f : f.name)
    : Object.entries(progression).filter(([k]) => k.includes("class_feature")).map(([, v]) => String(v));

  let lvl1ClassFeats: Array<{ name: string; level?: number; notes?: string; traits?: string }> = [];
  let higherClassFeats: Array<{ name: string; level?: number; notes?: string; traits?: string }> = [];
  ...
  const allLvl1Features = [
    ...classFeatures.map((f: string) => localizeCatalogItem(f, locale)),
  ...
  actionsList.slice(0, 4).forEach((act: any, idx: number) => {
    const n = idx + 1;
    ...
    const traits = Array.isArray(act.traits)
      ? act.traits.map((t: string) => localizeTrait(t, locale)).join(", ")
  ...
  reactionsList.slice(0, 4).forEach((react: any, idx: number) => {
    const n = idx + 1;
    ...
    const traits = Array.isArray(react.traits)
      ? react.traits.map((t: string) => localizeTrait(t, locale)).join(", ")
  ...
  if (isSpellcaster) {
    ...
    const abilityKey = ((character.spellcastingAbility || "int").slice(0, 3).toLowerCase() as keyof typeof mods);
